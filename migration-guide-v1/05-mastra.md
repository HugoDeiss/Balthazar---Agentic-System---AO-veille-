# Migration Guide v1.0 - Mastra

# Mastra Class | v1 Migration Guide

Mastra v1 Beta: Learn how to migrate Mastra class changes when upgrading to v1.

Source: https://mastra.ai/guides/v1/migrations/upgrade-to-v1/mastra
...
### Top-level imports to subpath importsDirect link to Top-level imports to subpath imports
...
```
- import { Mastra, Agent, Workflow, createTool } from '@mastra/core';+ import { Mastra, type Config } from '@mastra/core';+ import { Agent } from '@mastra/core/agent';+ import { Workflow } from '@mastra/core/workflows';+ import { createTool } from '@mastra/core/tools';
```
...
### experimental_auth to authDirect link to experimental_auth-to-auth
...
```
  const mastra = new Mastra({-   experimental_auth: {+   auth: {      provider: workos,    },  });
```
...
### Required id parameter for all mastra primitivesDirect link to required-id-parameter-for-all-mastra-primitives
...
```
- const storage = new LibSQLStore({-   url: ':memory:',- });+ const storage = new LibSQLStore({+   id: 'my-app-storage',+   url: ':memory:',+ });- const vector = new PgVector({-   connectionString: process.env.DATABASE_URL,- });+ const vector = new PgVector({+   id: 'my-app-vector',+   connectionString: process.env.DATABASE_URL,+ });
```

When using separate instances for different purposes, use descriptive unique IDs: 

```
  const agentMemory = new Memory({    storage: new LibSQLStore({-     url: 'file:./agent.db',+     id: 'weather-agent-memory-storage',+     url: 'file:./agent.db',    }),  });  const mastra = new Mastra({    storage: new LibSQLStore({+     id: 'mastra-storage',      url: ':memory:',    }),  });
```

### Primitive plural APIs changes from get<primitives> to list<primitive>Direct link to primitive-plural-apis-changes-from-getprimitives-to-listprimitive

`get*`functions that returned all instances of a primitive have been renamed to `list*`to better reflect their purpose. 

```
- const agents = mastra.getAgents();+ const agents = mastra.listAgents();- const vectors = mastra.getVectors();+ const vectors = mastra.listVectors();- const workflows = mastra.getWorkflows();+ const workflows = mastra.listWorkflows();- const scorers = mastra.getScorers();+ const scorers = mastra.listScorers();- const mcpServers = mastra.getMCPServers();+ const mcpServers = mastra.listMCPServers();- const logsByRunId = await mastra.getLogsByRunId({ runId: 'id', transportId: 'id' });+ const logsByRunId = await mastra.listLogsByRunId({ runId: 'id', transportId: 'id' });- const logs = await mastra.getLogs('transportId');+ const logs = await mastra.listLogs('transportId');
```
...
### Each primitive has a getById and a get functionDirect link to each-primitive-has-a-getbyid-and-a-get-function

```
mastra.getMCPServer('myServer');          // Works (registry key)mastra.getMCPServerById('my-mcp-server'); // Works (intrinsic ID)
```

### Tool registration now uses intrinsic IDsDirect link to Tool registration now uses intrinsic IDs
...
```
  const agent = new Agent({    id: 'agent1',    tools: {      searchTool: weatherSearchTool,    },  });- mastra.getTool('searchTool');+ mastra.getTool(weatherSearchTool.id);
```

## RemovedDirect link to Removed

### memory property from Mastra classDirect link to memory-property-from-mastra-class
...
```
- const mastra = new Mastra({-   memory: new Memory({-     storage: libsqlStore,-     vector: libsqlVector,-     embedder: fastembed,-   }),- });- - const agent = new Agent({-   name: 'my-agent',- });+ const mastra = new Mastra({+   // memory removed from here+ });+ + const agent = new Agent({+   name: 'my-agent',+   memory: new Memory({+     storage: libsqlStore,+     vector: libsqlVector,+     embedder: fastembed,+   }),+ });
```
